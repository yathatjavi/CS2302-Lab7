...
'''
will take a disjoint set forrest and returnt the directed adjacency matrix 
represention of that forrest
'''

def dsf_to_AM(tree):
    height = len(tree)
    draw_dsf(tree)
    AM = GS.Create_AM(height,height)
    for i in range(len(AM)):
        for j in range(len(AM[i])):
            if in_same_set(tree,i,j):
                AM[i][j] = True
...
'''
  will take a disjoint set forest and return an Adjaceny list representation
  of that forrest
  two if statments if to represent that there is a undirected path 
  between the two verticies that we are looking at.
'''

def DSF_to_AL(forrest):

    AL = Create_AL(len(forrest))
    for i in range(len(forrest)):
        if forrest[i] > 0:
            #if forrest[i] not in AL[i]:
            AL[i].append(forrest[i])
           # if i not in AL[forrest[i]]:
               # AL[forrest[i]].append(i)
    return AL
